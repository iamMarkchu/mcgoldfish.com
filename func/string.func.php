...
function my_substr($str, $len, $wholeWord=1, $append='...')
{
	$str = trim($str);
	$strLen = strlen($str);
	if($strLen <= $len)
	...
	{
		if($wholeWord == 1) //do not cut the word, identify by space
		{
			$prefix = substr($str, 0, $len);
			list($subfix, ) = split(' ', substr($str, $len));
			return $prefix.$subfix.$append;
		}
		else
		{
...
function filter_rewrite_tags($str)
{
	...
	return urlencode(my_trim($str));
}

function isManual($content) {
	if (strpos($content, 'm_') !== false) {
		return true;
	}else {
		return false;
	}
