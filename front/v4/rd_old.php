 ...
 */        
    ...
    if ($bz_type != 'PRINT' && stripos($bz_type, 'SHARE') === false) {
    	if(isset($store) && empty($store['aff']) && stripos($desturl, 'mgsvc.com') === false && stripos($desturl, "http") !== false) {
	    	$desturl = "http://mgsvc.com/?key=e4da3b7fbbce2345d7772b0674a318d5&id=" . urlencode(TRACKING_SITEID."_".$T_SERVERID."_".get_current_server_id()."_".$T_OUTGOINGID."_".$T_SESSION) . "&url=" . urlencode($desturl);
    	} else {
